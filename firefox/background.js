...
/* ── Clear persisted conversations on browser restart ── */
api.runtime.onStartup.addListener(() => {
  api.storage.local.remove('_conversations');
});
...
async function callChatGPTWeb(systemPrompt, userMessage) {
  ...
  if (!sessionRes.ok) throw new Error('Not logged in to ChatGPT. Open settings and log in.');
  ...
  let sentinelToken = '';
  try {
    const sRes = await fetch('https://chatgpt.com/backend-api/sentinel/chat-requirements', {
      method: 'POST', credentials: 'include',
      headers: { 'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`, 'oai-device-id': deviceId, 'oai-language': 'en-US' },
      body: '{}'
    });
    if (sRes.ok) { const s = await sRes.json(); sentinelToken = s.token || ''; }
  } catch { /* proceed without */ }

  const headers = {
    'Content-Type': 'application/json', 'Authorization': `Bearer ${token}`,
    'Accept': 'text/event-stream', 'oai-device-id': deviceId, 'oai-language': 'en-US'
  };
  ...
  const res = await fetch('https://chatgpt.com/backend-api/conversation', {
    ...
    body: JSON.stringify({
      action: 'next',
      messages: [{ id: crypto.randomUUID(), author: { role: 'user' }, content: { content_type: 'text', parts: [systemPrompt + '\n\n' + userMessage] } }],
      model: 'auto', parent_message_id: crypto.randomUUID(), conversation_mode: { kind: 'primary_assistant' }
    })
  });

  if (!res.ok) {
    const errText = await res.text().catch(() => '');
    if (res.status === 403) throw new Error('ChatGPT blocked this request. Try Claude or Gemini instead.');
  ...
  for (const line of lines) {
    try {
      ...
      if (parts?.length > 0 && typeof parts[0] === 'string') lastContent = parts[0];
    } catch { /* skip */ }
  }
  if (!lastContent) throw new Error('No response from ChatGPT.');
  return lastContent;
}
...
/* ── Direct: answer from current page content only ── */

async function handleDirect(question, pageData) {
  const settings = await getSettings();
  try {
    const answer = await callLLMRaw(settings,
      'Answer the user\'s question based only on the current page content below. Answer in English even if the content is in another language. Be concise and helpful.',
      `Page: ${pageData.title}\nURL: ${pageData.url}\n\n${pageData.ariaTree}\n\nQuestion: ${question}`
    );
    sendToSidePanel({
      type: 'answer',
      text: answer,
      question,
      sources: [{ url: pageData.url, title: pageData.title }]
    });
  } catch (err) {
    sendToSidePanel({ type: 'error', message: err.message || 'Failed.' });
  }
}
...
/* ── Compare: synthesize comparison of multiple tabs ── */

async function handleCompare(pages) {
  const settings = await getSettings();
  console.log('Compare: received', pages.length, 'pages, total chars:', pages.reduce((s, p) => s + (p.ariaTree || '').length, 0));

  try {
    sendToSidePanel({ type: 'progress', phase: 'Comparing pages...', detail: `${pages.length} pages` });

    const primary = pages.find(p => p.isPrimary) || pages[0];
    const others = pages.filter(p => p !== primary);

    const contentBlock = [
      `--- CURRENT PAGE: ${primary.title} (${primary.url}) ---\n${primary.ariaTree}`,
      ...others.map((p, i) => `--- COMPARISON ${i + 1}: ${p.title} (${p.url}) ---\n${p.ariaTree}`)
    ].join('\n\n');

    const systemPrompt = `The user is viewing "${primary.title}" and wants to compare it against ${others.length} other page${others.length > 1 ? 's' : ''}. Highlight key differences, similarities, pros and cons relative to the current page. Answer in English even if pages are in other languages. Structure the comparison clearly.`;

    const userPrompt = `${contentBlock}\n\n---\nCompare the current page against the others. What are the key differences?`;
    ...
    sendToSidePanel({ type: 'answer', text: answer, question: 'Compare tabs', sources });

  } catch (err) {
    console.error('Compare error:', err);
    sendToSidePanel({ type: 'error', message: err.message || 'Comparison failed.' });
  }
}
...
api.runtime.onMessage.addListener((msg, sender, sendResponse) => {
  if (msg.type === 'ask') {
    ...
    handleDirect(msg.question, msg.pageData);
